...
from src.models.spatial_excitaton import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='cge', num_groups=8, is3d=True, padding=1, **kwargs):
        ...
        if in_channels != out_channels:
            ...
            if is3d:
                self.conv1 = nn.Conv3d(in_channels, out_channels, 1, padding=padding)
            else:
                self.conv1 = nn.Conv2d(in_channels, out_channels, 1, padding=padding)
        else:
